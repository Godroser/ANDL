-- TPC-H Vector Queries for SeekDB
...
FROM
    orders
WHERE
    -- 修改点 1：去掉引号，确保 date 类型转换正确
    o_orderdate >= DATE '1993-07-01' 
    -- 修改点 2：使用更标准的 INTERVAL 语法
    ...
    (
        ...
        FROM
            ...
            LEFT OUTER JOIN orders ON 
                c_custkey = o_custkey
                -- 修改点 2：处理变量，移除占位符中的引号歧义
                AND o_comment NOT LIKE '%special%requests%'
        GROUP BY
            c_custkey
    ) AS c_orders  -- 修改点 3：移除 (c_custkey, c_count) 这种重命名方式
GROUP BY
    c_count
ORDER BY
    custdist DESC,
...
FROM
    ...
    part p
WHERE
    -- 将连接条件提取到外面，有助于优化器识别 Join 关系
    p.p_partkey = l.l_partkey
    AND (
        (
            p.p_brand = 'Brand#12'              -- 变量 :1
            AND p.p_container IN ('SM CASE', 'SM BOX', 'SM PACK', 'SM PKG')
            AND l.l_quantity >= 1               -- 变量 :4
        ...
        OR
        (
            p.p_brand = 'Brand#23'              -- 变量 :2
            AND p.p_container IN ('MED BAG', 'MED BOX', 'MED PKG', 'MED PACK')
            AND l.l_quantity >= 10              -- 变量 :5
        ...
        OR
        (
            p.p_brand = 'Brand#34'              -- 变量 :3
            AND p.p_container IN ('LG CASE', 'LG BOX', 'LG PACK', 'LG PKG')
            AND l.l_quantity >= 20              -- 变量 :6
...
FROM
    (
        SELECT
            -- 修改点 1：OceanBase 中 substring(str, pos, len) 更为通用
